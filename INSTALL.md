...
4. 根据**当前客户端**选择一种接入，不要全部装入用户的机器：
   - Pi：`pi install git:github.com/huaiwen/NetEaseEmailConnector@v0.2.0`，随后重启并用 `/skill:netease-email`。此路线使用 CLI，不需要 MCP adapter。
...
卸载：`uv tool uninstall netease-email-connector`；Pi 可 `pi remove git:github.com/huaiwen/NetEaseEmailConnector@v0.2.0`。只移除本项目服务/Skill，保留其他配置。用户邮箱配置不会自动删除，保留供重装使用。
