...
本指南用于用户已要求安装此连接器的本机 Agent。来源：https://github.com/huaiwen/NetEaseEmailConnector，版本 v0.2.2，MIT。不要更改无关客户端设置。
...
4. 根据**当前客户端**选择一种接入，不要全部装入用户的机器：
   - Pi：`pi install npm:netease-email-connector@0.2.2`，随后重启并用 `/skill:netease-email`。也可用 `pi install git:github.com/huaiwen/NetEaseEmailConnector@v0.2.2`。此路线使用 CLI，不需要 MCP adapter。
...
卸载：`uv tool uninstall netease-email-connector`；Pi 按安装来源执行 `pi remove npm:netease-email-connector@0.2.2` 或 `pi remove git:github.com/huaiwen/NetEaseEmailConnector@v0.2.2`。只移除本项目服务/Skill，保留其他配置。用户邮箱配置不会自动删除，保留供重装使用。
